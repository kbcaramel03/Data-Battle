...
OUTPUT_DIR = Path("xgboost_global_outputs")
...
df["icloud"] = df["icloud"].fillna(0).astype(int) 
df["is_last_lightning_cloud_ground"] = df["is_last_lightning_cloud_ground"].fillna(0).astype(int)
...
df["is_cg"] = (~df["icloud"].fillna(0).astype(bool)).astype(int)#nuage sol
df["is_ic"] = (df["icloud"].fillna(0).astype(bool)).astype(int)#intra nuage
...
for col in features:
    if str(model_df[col].dtype) in ["category", "object"]:
        # Il faut garder le type 'category' pour XGBoost
        model_df[col] = model_df[col].astype(str).fillna("MISSING").astype('category')
    else:
...
# TRAIN XGBOOST
# ============================================================
# Calcul du scale_pos_weight pour les classes déséquilibrées
# ratio = nombre d'exemples négatifs / nombre d'exemples positifs
...
# Attention : l'entraînement XGBoost avec early_stopping se fait comme ceci :
model.fit(
    X_train, y_train,
